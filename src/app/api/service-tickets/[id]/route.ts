import { NextRequest, NextResponse } from "next/server";
import { prisma } from "@/lib/prisma";

// GET /api/service-tickets/[id]
export async function GET(
  request: NextRequest,
  { params }: { params: Promise<{ id: string }> }
) {
  try {
    const { id } = await params;
    const ticket = await prisma.serviceTicket.findUnique({ where: { id } });

    if (!ticket) {
      return NextResponse.json({ error: "Service ticket not found" }, { status: 404 });
    }

    return NextResponse.json(ticket);
  } catch (error) {
    console.error("GET /api/service-tickets/[id] error:", error);
    return NextResponse.json({ error: "Failed to fetch service ticket" }, { status: 500 });
  }
}

// PATCH /api/service-tickets/[id]
export async function PATCH(
  request: NextRequest,
  { params }: { params: Promise<{ id: string }> }
) {
  try {
    const { id } = await params;
    const body = await request.json();

    const existing = await prisma.serviceTicket.findUnique({ where: { id } });
    if (!existing) {
      return NextResponse.json({ error: "Service ticket not found" }, { status: 404 });
    }

    const updateData: Record<string, unknown> = { ...body };
    if (updateData.visitDate && typeof updateData.visitDate === "string") {
      updateData.visitDate = new Date(updateData.visitDate);
    }
    if (updateData.followUpDate && typeof updateData.followUpDate === "string") {
      updateData.followUpDate = new Date(updateData.followUpDate);
    }

    // Auto-set closedAt when status changes to CLOSED
    if (updateData.status === "CLOSED" && existing.status !== "CLOSED") {
      updateData.closedAt = new Date();
    }

    const ticket = await prisma.serviceTicket.update({
      where: { id },
      data: updateData,
    });

    return NextResponse.json(ticket);
  } catch (error) {
    console.error("PATCH /api/service-tickets/[id] error:", error);
    return NextResponse.json({ error: "Failed to update service ticket" }, { status: 500 });
  }
}

// DELETE /api/service-tickets/[id]
export async function DELETE(
  request: NextRequest,
  { params }: { params: Promise<{ id: string }> }
) {
  try {
    const { id } = await params;
    const ticket = await prisma.serviceTicket.findUnique({ where: { id } });
    if (!ticket) {
      return NextResponse.json({ error: "Service ticket not found" }, { status: 404 });
    }
    await prisma.serviceTicket.delete({ where: { id } });
    return NextResponse.json({ success: true });
  } catch (error) {
    console.error("DELETE /api/service-tickets/[id] error:", error);
    return NextResponse.json({ error: "Failed to delete service ticket" }, { status: 500 });
  }
}
